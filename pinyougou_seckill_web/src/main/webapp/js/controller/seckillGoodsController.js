...
app.controller('seckillGoodsController', function ($scope, $controller, $interval, $location, seckillGoodsService) {
    ...
    $scope.findList = function () {
        seckillGoodsService.findList().success(function (responces) {
            $scope.list = responces;
        })
    }

    //查询单个秒杀商品
    $scope.findOneFromRedis = function () {
        seckillGoodsService.findOneFromRedis($location.search()['seckillId']).success(function (responces) {
            $scope.entity = responces;
            allsecond = Math.floor(new Date(new Date(responces.endTime).getTime() - new Date().getTime()) / 1000);
